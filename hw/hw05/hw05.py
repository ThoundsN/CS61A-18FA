...
def balanced(m):
    ...
    if is_weight(m):
        ...
        leftside, rightside = left(m), right(m)
        lefttorque = length(leftside)*total_weight(end(leftside))
        righttorque = length(rightside)*total_weight(end(rightside))
        return balanced(end(leftside)) and balanced(end(rightside)) and lefttorque==righttorque
...
def totals_tree(m):
    ...
    if is_weight(m):
        ...
        branches = [totals_tree(leftend)] + [ totals_tree(rightend)]
...
def make_withdraw(balance, password):
    ...
    def withdraw(amount,inputpass):
        nonlocal balance,password,times
        if times >= 3:
            return "Your account is locked. Attempts: " + str(wrongpass)
        if inputpass != password:
            times += 1
            wrongpass.append(inputpass)
