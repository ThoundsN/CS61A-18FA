...
class BodyguardAnt(Ant):
    ...
    # def reduce_armor(self, amount):
    #     if amount >= self.armor and self.contained_ant is not None:
    #         place = self.place
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while place.exit is not None:
            curant = place.ant
            if curant :
                if hasattr(curant,'damage') and curant not in self.ants_have_been_doubled:
                    curant.damage *= 2
                    self.ants_have_been_doubled.append(curant)
                if curant.is_container and curant.contained_ant:
                    if curant.contained_ant not in self.ants_have_been_doubled :
                        curant.contained_ant.damage *=2
                        self.ants_have_been_doubled.append(curant.contained_ant)
            place = place.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        # BEGIN Problem 13
        "*** YOUR CODE HERE ***"
        if amount >= self.armor and self.truequeen is True:
            bees_win()
        else:
...
def make_scare(action, bee):
    ...
    if bee.havebeenscare is False:
        bee.havebeenscare = True
