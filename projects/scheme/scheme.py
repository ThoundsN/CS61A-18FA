...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        evalone = lambda a: scheme_eval(a,env)
        arguments = rest.map(evalone)
...
class Frame(object):
    ...
    def make_child_frame(self, formals, vals):
        ...
        if len(formals) != len(vals):
            raise SchemeError("values and arguments fucking don't match ")
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target):
        ...
        # BEGIN PROBLEM 10
        "*** YOUR CODE HERE ***"
        # allready contain nil in expression.second
        lambdaexpr = Pair(target.second,expressions.second)
        procedure = do_lambda_form(lambdaexpr, env)
...
def make_let_frame(bindings, env):
    ...
    while bindings is not nil:
        ...
        values = scheme_eval(bindings.first.second.first,env)
        formalset = Pair(formals,formalset)
        valueset = Pair(values,valueset)
        bindings = bindings.second
    check_formals(formalset)
    return env.make_child_frame(formalset,valueset)
    # END PROBLEM 15
