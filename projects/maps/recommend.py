...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** YOUR CODE HERE ***"
    def find_closest_centroid(r):
        return find_closest(restaurant_location(r), centroids)
    groups = [[find_closest_centroid(r),r] for r in restaurants]
...
def find_centroid(cluster):
    ...
    # BEGIN Question 5
    "*** YOUR CODE HERE ***"
    location = [restaurant_location(r) for r in cluster]
    xset = [l[0]  for l in location]
    yset = [l[1]  for l in location]
    x, y = mean(xset), mean(yset)
    return  [x,y]
         # END Question 5
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    "*** YOUR CODE HERE ***"
    func  = max(feature_fns,key = lambda fn: find_predictor(user,reviewed,fn)[1] )
...
def rate_all(user, restaurants, feature_fns):
    ...
    for r in restaurants:
        if r in reviewed:
            new = {restaurant_name(r):user_rating(user,restaurant_name(r))}
            rate_dic.update(new)
        else:
            new = {restaurant_name(r):predictor(r)}
            rate_dic.update(new)
